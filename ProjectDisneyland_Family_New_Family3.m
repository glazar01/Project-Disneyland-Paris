...
for k = 1 : 3
    ...
    if (k ==1)
        Tmax = 600; % Tmax for the family as a whole --> (Phase 1)
    else
        Tmax = 0; % Tmax for the part that they are separated --> (Phase 2)
    end
    ...
    % --------------------------------------------------------------------
    RateGroup = 12;          % Whole family rating - used in objective phase 1
    RateGroup1 = 1;         % Rate for Age1        
    RateGroup2 = 2;         % Rate for Age2
    RateGroup3 = 5;         % Rate for Age3
    RateGroup4 = 4;         % Rate for Age4
    
    RateGroup5 = 15;        % Rate for Age1 + Age2 + Age3 (for first group k = 2)
    RateGroup6 = 4;         % Rate for Age4 (for second group k = 3)
    ...
    Ratings = csvread('RatingsModified4.csv');      % Columns: Preschoolers | Kids | Tweens | Teens | Adults | Kid+Teen+Adult | Kid & Adults | Preschooler & Adult | Teen & Kid | Tween & Adult | Preschooler&Teen&Adult | Preschooler&Kid&Teen&Adult | Preschooler+Tween+Adult | Kid&Tween&Adult | Kid+Preschooler+Adult
    ...
    for i = 1 : N                                                                                                               % Constraints for age restrictions
        ...
        if(k == 2)
            if (AgeRes1(i) == 1 || AgeRes2(i) == 1 || AgeRes3(i) == 1)
                for j = 1 : N+1
                    Constraints = [Constraints, x(j,i+1) == 0, x(i+1,j+1) == 0];
                end
            end
    ...
    if sol.problem == 0
        ...
        if(k == 2)
            ...
            solution_age1_age2_age3 = value(x)
            Total_Rate_Age1 = Total_Rate_Age1 + sum(sum(Games*Rate1));
            Total_Rate_Age2 = Total_Rate_Age2 + sum(sum(Games*Rate2));
            Total_Rate_Age3 = Total_Rate_Age3 + sum(sum(Games*Rate3));
        end
